...
def _initialize_sheets(credentials: Any) -> Any:
    ...
    print(f"Building sheets service with credentials type: {type(credentials)}")
...
def get_sheet(credentials: Any, spreadsheet_id: str, sheet_name: str) -> Iterator:
    ...
    print(f"Fetching sheet: ID={spreadsheet_id}, Name={sheet_name}")
    try:
        result = (
            sheets.spreadsheets()
            .values()
        ...
        print(f"Retrieved {len(values)} rows from the sheet")
...
def get_credentials(credentials_info: str | dict) -> Any:
    print(f"Getting credentials from: {credentials_info}")
...
if __name__ == "__main__":
    ...
    print(f"Credentials Path: {CREDENTIALS_INFO}")
    print(f"Salt is set: {'Yes' if SALT else 'No'}")
